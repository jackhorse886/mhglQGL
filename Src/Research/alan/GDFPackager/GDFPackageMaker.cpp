...
// Public API
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID GDFPackageMaker::Add(CONST String SrcFilename)
{
	WIN32_FIND_DATA FileData; 
	HANDLE hSearch = FindFirstFile(SrcFilename.c_str(), &FileData);
	if (hSearch != INVALID_HANDLE_VALUE)
	{
		...
		{ 
			...
			if (!FindNextFile(hSearch, &FileData)) 
			{
				if (GetLastError() == ERROR_NO_MORE_FILES) 
					...
					TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFPackageMaker::Add Error (1) [Next file find error] filename=%s"), SrcFilename.c_str());
			}
		...
		for( Index i = C_INDEX(0); i < fileVector.size(); ++i )
		{
			...
			{
				AddArchiveChange(filename, MPQ_ARCHIVE_CHANGE_SOURCE_FILE);
			}
		}
		FindClose(hSearch);
	}else
	{
		...
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFPackageMaker::Add Error (2) [File not found] filename=%s"), SrcFilename.c_str());
	}
}

VOID GDFPackageMaker::RecursiveAdd(CONST String SrcFilename)
{
	String TargetInput;
	TargetInput.Format(_T("%s\\*.*"), SrcFilename.c_str());
	...
	while (finder.GetPathName(&FullPath) != 0)
	{
		...
		if( attr & FILE_ATTRIBUTE_DIRECTORY) // if directory, add all files inside; if not, add only source file.
		{
			RecursiveAdd(FullPath);
		}else
			AddArchiveChange(FullPath, MPQ_ARCHIVE_CHANGE_SOURCE_FILE);
	}
}

VOID GDFPackageMaker::UpdatePatch(CONST String SrcFilename, BOOL detailCheck)
{
	Add(SrcFilename);		// a list of updates is prepared
	...
	for( Index i = C_INDEX(0); i < m_archiveChange.size(); ++i )
	{
		...
		history		= GetHistoryData(storeString);
		if( history )
		{
			...
			{
				checksum1 = 0;
				Reuben::Math::CRC32::Calculate(storeString.c_str(), checksum1);
	...
	for( Index i = C_INDEX(0); i < m_archiveChange.size(); ++i )
	{
		if( m_archiveChange[i]->type==MPQ_ARCHIVE_CHANGE_UPDATE || m_archiveChange[i]->type==MPQ_ARCHIVE_CHANGE_ADD )
		{
			...
			if( history )
			{
				...
				Reuben::Math::CRC32::Calculate(storeString.c_str(), history->filechecksum);
			}else
			{
...
GdfHistoryData* GDFPackageMaker::GetHistoryData(String& SrcFilename)
{
	for( Index i = C_INDEX(0); i < m_historyData.size(); ++i )
	{
		TCHAR* tptr = m_historyData[i]->filename;
		if( m_historyData[i] && SrcFilename.compare(tptr)==0 )
...
VOID GDFPackageMaker::AddArchiveChange(CONST String SrcFilename, UInt8 SrcType)
{
...
BOOL GDFPackageMaker::Extract(String& patchFilename, String& targetFilename)
{
	BOOL succeed = TRUE;
	...
	if( mgr.LoadPatchFile(patchFilename, GDFFILE_STATE_GAME) )
	{
		...
		UInt32 fileSize		= 0;
		BYTE* buf;
		BYTE* mapview = mgr.GetMapViewOfFile(targetFilename, beginOffset, fileSize);
		if( mapview )
		{
			...
			File file;
			file.Open(targetFilename.c_str(), File::MODE_WRITE);
			if (file.IsOpened())
			{
			...
			{
				TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFPackageMaker::Extract Error (2) [Write file error] patchFilename=%s, targetFilename=%s"), patchFilename.c_str(), targetFilename.c_str());
				succeed = FALSE;
			}
			UnmapViewOfFile(mapview);
		}else
		{
			TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFPackageMaker::Extract Error (3) [Target File not found in patch] patchFilename=%s, targetFilename=%s"), patchFilename.c_str(), targetFilename.c_str());
			succeed = FALSE;
		}
		mgr.ClosePatchFile();
	}else
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFPackageMaker::Extract Error (4) [File open error] patchFilename=%s"), patchFilename.c_str());
		succeed = FALSE;
	}
	return succeed;
}

BOOL GDFPackageMaker::ExtractAll(String& patchFilename)
{
	BOOL succeed = TRUE;
	...
	for( Index i = C_INDEX(0); i < (UInt32)list.size(); ++i )
	{
		...
		if( name )
		{
			...
			fstream = mgr.GetStream(name->filename);
			if( fstream )
			{
				// Recursive Directory Create
				{
				...
				try{
					...
					{
						DEBUG_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFPackageMaker::ExtractAll Error (1) [File write open error] patchFilename=%s, extractName=%s"), patchFilename.c_str(), name->filename.c_str());
						succeed = FALSE;
					}
				}catch(FileException Er)
				{
					DEBUG_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFPackageMaker::ExtractAll Error (2) [File write error catch] patchFilename=%s, extractName=%s"), patchFilename.c_str(), name->filename.c_str());
					succeed = FALSE;
				}
				SafeDeleteObject(fstream);
			}else
			{
				DEBUG_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFPackageMaker::ExtractAll Error (3) [File not found in patch] patchFilename=%s, extractName=%s"), patchFilename.c_str(), name->filename.c_str());
				succeed = FALSE;
			}
		}else
		{
			DEBUG_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFPackageMaker::ExtractAll Error (4) [File list preparation error] patchFilename=%s"), patchFilename.c_str());
			succeed = FALSE;
		}
	}
...
VOID GDFPackageMaker::Delete(CONST String TrgFilename, BOOL IsDirectory)
{
	if( IsDirectory ) // if directory, delete all files inside; if not, delete only source file.
	{
		String cTrgFilename = TrgFilename.c_str();
...
VOID GDFPackageMaker::DeleteArchiveChange(CONST String SrcFilename)
{
	GdfArchiveChange* temp = SafeCreateObject(GdfArchiveChange);
	temp->filename = SrcFilename;
...
BOOL GDFPackageMaker::View(String& patchFilename)
{
	if( !File::IsExists(patchFilename.c_str()) )
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFPackageMaker::View Error (1) [File not found] patchFilename=%s"), patchFilename.c_str());
		return FALSE;
	}
	...
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFPackageMaker::View Error (2) [load patch file failed] patchFilename=%s"), patchFilename.c_str());
		return FALSE;
	}
...
BOOL GDFPackageMaker::Test(String& patchFilename)
{
	if( !File::IsExists(patchFilename.c_str()) )
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFPackageMaker::Test Error (1) [File not found] patchFilename=%s"), patchFilename.c_str());
		return FALSE;
	}
	...
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFPackageMaker::Test Error (2) [testpatchfile failed] patchFilename=%s"), patchFilename.c_str());
		return FALSE;
	}
	return TRUE;
}

BOOL GDFPackageMaker::Repatch(String& patchFilename)
{
	if( !File::IsExists(patchFilename.c_str()) )
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFPackageMaker::Repatch Error (1) [File not found] patchFilename=%s"), patchFilename.c_str());
		return FALSE;
	}
	...
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFPackageMaker::Repatch Error (2) [repatch failed] patchFilename=%s"), patchFilename.c_str());
		return FALSE;
	}
	return TRUE;
}
...
BOOL GDFPackageMaker::ExecuteFileUpdate(String& patchFilename, UInt32 writeLogVersion, BOOL ForceMaster)
{
	if( m_archiveChange.size()==0 && File::IsExists(patchFilename.c_str()) )		return TRUE;

	BOOL succeed;
	GDFManager mgr;
	succeed = mgr.UpdatePatch(patchFilename, m_archiveChange, writeLogVersion, ForceMaster);
	return succeed;
}

BOOL GDFPackageMaker::ExecuteMerge(String& ToFilename, String& FromFilename, BOOL inheritPatchStep)
{
	if( ToFilename.CompareNoCase(FromFilename.c_str())==0 )
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFPackageMaker::ExecuteMerge (1) [same data source] Filename=%s"), FromFilename.c_str());
		return FALSE;
	}

	BOOL succeed;
	GDFManager mgr;
	succeed = mgr.Merge(ToFilename, FromFilename, inheritPatchStep);
	return succeed;
}


BOOL GDFPackageMaker::ViewPatchStep(String& patchFilename)
{
	if( !File::IsExists(patchFilename.c_str()) )
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFPackageMaker::ViewPatchStep (1) [File not found] patchFilename=%s"), patchFilename.c_str());
		return FALSE;
	}
	...
	if( !mgr.LoadPatchFile(patchFilename, GDFFILE_STATE_PATCH_READ) )
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFPackageMaker::ViewPatchStep (2) [load patch file failed] patchFilename=%s"), patchFilename.c_str());
		return FALSE;
	}
