...
#define UPDATE_PATCHER_FILENAME				_T("QGLPatcher.EXE")
#define UPDATE_PATCHER_FILENAME_SRC			_T("patch\\trash\\QGLPatcher.EXE")
...
BOOL CUpdateApp::LoadConfiguration()
{
	...
	GetPrivateProfileString(_T("APP"), _T("startrestart"), _T(""), tmpstr, 260, CONFIGURATION_INI_FILE);
	str = tmpstr;
	SetPatcherFilename(str);
	GetPrivateProfileString(_T("APP"), _T("startgame"), _T(""), tmpstr, 260, CONFIGURATION_INI_FILE);
	str = tmpstr;
	SetStartGameFilename(str);
	return TRUE;
}