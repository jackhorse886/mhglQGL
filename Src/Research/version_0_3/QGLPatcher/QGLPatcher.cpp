...
BOOL CQGLPatcherApp::ReadPatchCurrentVersion()
{
	...
	m_curVersion.Format(_T("%s%s"), VERSION_HEADER, pBuf);
	UInt ver = _ttoi(pBuf);
	if( ver==0 )
		return FALSE;
...
BOOL CQGLPatcherApp::DownloadPatchSummary()
{
	...
	while( !(bSumSuccess && bFileSuccess) && GetSummarySelection()!=-1 )
	{
		...
		while(true)
		{
			...
			case 1:	// HTTP
				{
						...
						if( pHttpConnection )
						{
							...
							if( (msg+1)/100==2 )
							{
								...
								if( filesize > 0 )
								{
									...
									{
										TRACE_ERRORDTL(GLOBAL_LOGGER, _T("CQGLPatcherApp::DownloadPatchSummary (1) [File exception no Summary HTTP connection]"));
										bSumSuccess = FALSE;;
										file.Close();
									}
									SafeDeallocate(buf);
					...
					try{
						...
						if( pHttpConnection )
						{
							...
							{
								TRACE_ERRORDTL(GLOBAL_LOGGER, _T("CQGLPatcherApp::DownloadPatchSummary (3) [exception caught using HTTP connection]"));
								bFileSuccess = FALSE;
							}
							pFile1->Close();
							pHttpConnection->Close();
					...
					{
						TRACE_ERRORDTL(GLOBAL_LOGGER, _T("CQGLPatcherApp::DownloadPatchSummary (4) [exception caught using HTTP connection]"));
						bFileSuccess = FALSE;
					}
			...
			if( !(bSumSuccess && bFileSuccess) && retries < MAX_LIMIT_TRY )
			{
				retries++;
				Sleep(MAX_LIMIT_DELAY);
			}else
		...
		case 2: // FTP
			{
				for( Index i = C_INDEX(0); i < m_patchFileSummary.size(); ++i )
				{
					if( m_patchFileSummary[i].version==GetPatchCurrentVersion() )
					{
						...
						BOOL bSuccess = FTPDownload(strIP.c_str(), uPort, strUsername.c_str(), strPassword.c_str(), strPath.c_str(), strFilename.c_str(), strLocalFilename.c_str());
...
BOOL CQGLPatcherApp::LoadSummaryConfig()
{
	...
	m_serVersion.Format(_T("%s%i"), VERSION_HEADER, GetPatchTargetVersion());
...
{
	if( m_serverSummaryList.size()==0 || m_serverFileList.size()==0 )	// invalid input, return failed
		return -1;
	if( m_serverSummaryList.size() != m_serverFileList.size() )
		return -1;
