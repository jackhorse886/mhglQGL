	...
	case 'a':
		{
			if( param1.Compare(_T("-master"))==0 )
			{
				if( !param2.IsEmpty() && !param3.IsEmpty() )
				{
					...
					packageMaker.Add(param3);
					packageMaker.ExecuteFileUpdate(param2, 0, TRUE);
				}else
				{
					DisplayUsage();
				}
			}else if( param1.GetAt(0)=='-' && param1.Left(8).Compare(_T("-version"))==0 )	// patch step is recorded
			{
				if( !param2.IsEmpty() && !param3.IsEmpty() )
				{
					...
					}else
					{
						GDFPackageMaker packageMaker;
						packageMaker.Add(param3);
						packageMaker.ExecuteFileUpdate(param2, scriptVersion, FALSE);
					}
				}else
				{
					DisplayUsage();
			...
			}else
			{
				...
				{
					GDFPackageMaker packageMaker;
					packageMaker.Add(param2);
					packageMaker.ExecuteFileUpdate(param1, 0, FALSE);
				}else
				{
					DisplayUsage();
				}
	...
	case 'd':
		{
			if( param1.GetAt(0)=='-' && param1.Left(8).Compare(_T("-version"))==0 )
			{
				...
				}else
				{
					...
					{
						GDFPackageMaker packageMaker;
						packageMaker.Delete(param3, FALSE);
						packageMaker.ExecuteFileUpdate(param2, scriptVersion);
					}else
					{
						DisplayUsage();
					}
			...
			}else
			{
				...
				{
					GDFPackageMaker packageMaker;
					packageMaker.Delete(param2, FALSE);
					packageMaker.ExecuteFileUpdate(param1, 0);
				}else
				{
					DisplayUsage();
				}
	...
	case 'e':
		{
			if( param1.Compare(_T("-all"))==0 )
			{
				...
				{
					GDFPackageMaker packageMaker;
					packageMaker.ExtractAll(param2);
				}else
				{
					DisplayUsage();
				}
			}else
			{
				...
				{
					GDFPackageMaker packageMaker;
					packageMaker.Extract(param1, param2);
				}else
				{
					DisplayUsage();
				}
	...
	case 'm':
		{
			if( param1.GetAt(0)=='-' && param1.Left(6).Compare(_T("-patch"))==0 )
			{
				...
				{
					GDFPackageMaker packageMaker;
					packageMaker.ExecuteMerge(param2, param3, TRUE);	// patch file merge -> patch steps must be emrged
				}else
				{
					DisplayUsage();
				}
			}else
			{
				...
				{
					GDFPackageMaker packageMaker;
					packageMaker.ExecuteMerge(param1, param2, FALSE);	// patch-to-master file merge -> patch steps no use after merge
				}else
				{
					DisplayUsage();
				}
	...
	case 'r':
		{
			...
			{
				GDFPackageMaker packageMaker;
				if( !packageMaker.Repatch(param1) )
				{
	...
	case 'u':
		{
			if( param1.Compare(_T("-master"))==0 )
			{
				if( param2.Compare(_T("-f"))==0 )
				{
					...
					{
						GDFPackageMaker packageMaker;
						packageMaker.UpdatePatch(param4, TRUE);
						packageMaker.ExecuteFileUpdate(param3, 0, TRUE);
				...
				{
					GDFPackageMaker packageMaker;
					packageMaker.UpdatePatch(param3, FALSE);
					packageMaker.ExecuteFileUpdate(param2, 0, TRUE);
					packageMaker.WriteHistoryFile();
				}else
				{
			...
			}else if( param1.Compare(_T("-f"))==0 )
			{
				if( param2.GetAt(0)=='-' && param2.Left(8).Compare(_T("-version"))==0 )	// patch step is recorded
				{
					if( !param3.IsEmpty() && !param4.IsEmpty() )
					{
						...
						}else
						{
							GDFPackageMaker packageMaker;
							packageMaker.UpdatePatch(param4, TRUE);
							packageMaker.ExecuteFileUpdate(param3, scriptVersion, FALSE);
							packageMaker.WriteHistoryFile();
						}
					}else
					{
				...
				}else
				{
					if( !param2.IsEmpty() && !param3.IsEmpty() )
					{
						GDFPackageMaker packageMaker;
						packageMaker.UpdatePatch(param3, FALSE);
						packageMaker.ExecuteFileUpdate(param2, 0, FALSE);
						packageMaker.WriteHistoryFile();
					}else
					{
						DisplayUsage();
			...
			}else
			{
				...
				{
					GDFPackageMaker packageMaker;
					packageMaker.UpdatePatch(param2, FALSE);
					packageMaker.ExecuteFileUpdate(param1, 0, FALSE);
					packageMaker.WriteHistoryFile();
				}else
				{
	...
	case 'v':
		{
			if( !param1.IsEmpty() )
			{
				...
				{
					GDFPackageMaker packageMaker;
					if( !packageMaker.ViewPatchStep(param2) )
					{
...
void InitLogger()
{
	...
	gGlobalLogger = SafeCreateObject2(Logger, _T("QGLPatcher"), CONFIGURATION_INI_FILE);
