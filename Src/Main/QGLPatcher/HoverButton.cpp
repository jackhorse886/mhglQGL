...
BOOL CHoverButton::LoadBitmap(PCSTRING bmp_normal, PCSTRING bmp_hover, PCSTRING bmp_down)
{
	BOOL succeeded	= TRUE;
	if( mybitmap )
	{
	...
	if (!LoadFileBitmap(mybitmap, bmp_normal))
	{
		delete mybitmap;
		mybitmap	= NULL;
		succeeded	= FALSE;
	}
	if (!LoadFileBitmap(mybitmaphover, bmp_hover))
	{
		delete mybitmaphover;
		mybitmaphover	= NULL;
		succeeded		= FALSE;
	}
	if (!LoadFileBitmap(mybitmapdown, bmp_down))
	{
		delete mybitmapdown;
		mybitmapdown	= NULL;
		succeeded		= FALSE;
...
BOOL CHoverButton::ReplaceBitmap(PCSTRING bmp_normal, PCSTRING bmp_hover, PCSTRING bmp_down)
{
	BOOL succeeded = TRUE;
	if( mybitmap )
	{
