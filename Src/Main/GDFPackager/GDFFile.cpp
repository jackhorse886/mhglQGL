...
{
    DEBUG_ENSURE(lpszString);
    DEBUG_ENSURE(dwHashType <= MPQ_HASH_FILE_KEY);
...
{
	if( patchstep.size() > 0 )	// patch step mode
	{
		for( PatchStepMap::iterator itr = patchstep.begin(); itr!=patchstep.end(); ++itr )
		{
			...
			{
				if( itr->second[i]->type==MPQ_ARCHIVE_CHANGE_ADD && IsGdfFileExist(itr->second[i]->filename.c_str()) )
			...
			if( itr->second.size() + m_listFile.size() > 0 && itr->second.size() + m_listFile.size() + 2 > m_hashTable.size() )	// (listfile) + (patchstep)
			{
				...
				if( newhashsize > hashsize )
				{
					...
					if( curPower )
					{
						...
						{
							TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::SavePatchData Error (5A) [increase hash failed]"));
							return FALSE;
						}
					}else
					{
						TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::SavePatchData Error (4A) [increase hash, but cur. hash calc. error] newhashsize=%i"), newhashsize);
						return FALSE;
					}
				}
			}
			...
			for( Index i = C_INDEX(0); i < itr->second.size(); ++i )
			{
				if( itr->second[i] && itr->second[i]->type==MPQ_ARCHIVE_CHANGE_DELETE )
				{
					...
					{
						GdfListFile* tmplist;
						for( ListFileVector::iterator itr2 = m_listFile.begin(); itr2 != m_listFile.end(); )
						{
							tmplist = *itr2;
							if( tmplist )
							{
								if( tmplist->filename.CompareNoCase(itr->second[i]->filename.c_str())==0 )
								{
									SafeDeleteObject(tmplist);
									m_listFile.erase(itr2);
									itr2 = m_listFile.begin();
								}else
								{
									++itr2;
								}
					...
					{
						TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::SavePatchData Error (1A) [Delete File error] filename=%s"), itr->second[i]->filename.c_str());
						return FALSE;
					}
					if( mergePatchStep )
						PatchStepDelete(itr->first, itr->second[i]->filename.c_str());
				}
			}
			...
			for( Index i = C_INDEX(0); i < itr->second.size(); ++i )
			{
				if( itr->second[i] )
				{
					...
					case MPQ_ARCHIVE_CHANGE_UPDATE:
						{
							GDFStream* fstream = SrcGdfFile->GetStream(itr->second[i]->filename.c_str());
							if( fstream )
							{
								if( !InsertFileFromStream(m_file, itr->second[i]->filename.c_str(), fstream) )
								{
									TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::SavePatchData Error (2A) [Insert File Stream error] filename=%s"), itr->second[i]->filename.c_str());
									return FALSE;
								}
								if( mergePatchStep )
									PatchStepAdd(itr->first, itr->second[i]->filename.c_str());
							}else
							{
								TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::SavePatchData Error (3A) [source stream not found] filename=%s"), itr->second[i]->filename.c_str());
								return FALSE;
							}
						}
						break;
			...
			if(!InsertFileFromMemory(m_file, listfile_filename.c_str(), (BYTE*)tempbuf, listfile_tLength * sizeof(TCHAR), FALSE))
			{
				TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::SavePatchData Error (6A) [list file save error]"));
				return FALSE;
			}
			...
			if(!InsertFileFromMemory(m_file, patchstep_filename.c_str(), (BYTE*)tempstepbuf, patchstep_tLength * sizeof(TCHAR), FALSE))
			{
				TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::SavePatchData Error (7B) [(patchstepfile) save error]"));
				return FALSE;
			}
	...
	{
		ListFileVector listvec = SrcGdfFile->GetListFile();
		ArchiveChangeVector change;
		for( Index i = C_INDEX(0); i < listvec.size(); ++i )
		{
			GdfArchiveChange* cha = SafeCreateObject(GdfArchiveChange);
			cha->filename		= listvec[i]->filename;
			cha->type			= MPQ_ARCHIVE_CHANGE_ADD;
			change.push_back(cha);
		}
		// preprocess Changes
		for( Index i = C_INDEX(0); i < change.size(); ++i )
		{
			if( change[i]->type==MPQ_ARCHIVE_CHANGE_ADD && IsGdfFileExist(change[i]->filename.c_str()) )
				change[i]->type = MPQ_ARCHIVE_CHANGE_UPDATE;
		}

		// new hash size check
		if( change.size() + m_listFile.size() > 0 && change.size() + m_listFile.size() + 2 > m_hashTable.size() )	// (listfile) + (patchstep)
		{
			Int32 counter = 0;
			for( Index i = C_INDEX(0); i < change.size(); ++i )
			{
				switch( change[i]->type )
				{
				case MPQ_ARCHIVE_CHANGE_ADD:	counter++;	break;
				}
			}
		...
		for( Index i = C_INDEX(0); i < change.size(); ++i )
		{
			if( change[i] )
			{
				...
				case MPQ_ARCHIVE_CHANGE_UPDATE:
					{
						GDFStream* fstream = SrcGdfFile->GetStream(change[i]->filename.c_str());
						if( fstream )
						{
							if( !InsertFileFromStream(m_file, change[i]->filename.c_str(), fstream) )
							{
								TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::SavePatchData Error (2A) [Insert File Stream error] filename=%s"), change[i]->filename.c_str());
								return FALSE;
							}
						}else
						{
							TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::SavePatchData Error (3A) [source stream not found] filename=%s"), change[i]->filename.c_str());
							return FALSE;
						}
					}
					break;
...
BOOL GDFFile::SavePatchData(ArchiveChangeVector& change, PCSTRING srcFolder, UInt32 writeLogVersion)
{
	// preprocess Changes
	for( Index i = C_INDEX(0); i < change.size(); ++i )
	{
		GdfArchiveChange* ac = change[i];
		if( ac && ac->type==MPQ_ARCHIVE_CHANGE_ADD && IsGdfFileExist(ac->filename.c_str()) )
	...
	if( m_listFile.size() + change.size() > 0 && m_listFile.size() + change.size() + 2 > m_hashTable.size() )	// (listfile) + (patchstep)
	{
		...
		if( newhashsize > hashsize )
		{
			...
			if( curPower )
			{
				if( !IncreaseHash(curPower) )
				{
					TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::SavePatchData Error (5B) [increase hash failed]"));
					return FALSE;
				}
			}else
			{
				TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::SavePatchData Error (4B) [increase hash, but cur. hash calc. error] newhashsize=%i"), newhashsize);
				return FALSE;
			}
		}
	}
		...
		if( archiveChange && archiveChange->type==MPQ_ARCHIVE_CHANGE_DELETE )
		{
			if( DeleteFile(archiveChange->filename.c_str(), MPQNeutral, 0) )
			{
				...
				for( ListFileVector::iterator itr2 = m_listFile.begin(); itr2 != m_listFile.end(); )
				{
					...
					if( tmplist )
					{
						if( tmplist->filename.CompareNoCase(archiveChange->filename.c_str())==0 )
						{
							SafeDeleteObject(tmplist);
							m_listFile.erase(itr2);
							itr2 = m_listFile.begin();
			...
			}else
			{
				...
				{
					TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::SavePatchData Error (1B) [Delete File error] filename=%s"), archiveChange->filename.c_str());
					return FALSE;
				}
			}
			PatchStepDelete(writeLogVersion, archiveChange->filename.c_str());
		}
	}
	String fileFullPath, fileRelativePath;
	m_file.SetPosition(m_header->hashTableOffset, Reuben::Platform::File::MOVE_BEGIN);
	for( Index i = C_INDEX(0); i < change.size(); ++i )
	{
		GdfArchiveChange* archiveChange = change[i];
		if( srcFolder )
			fileFullPath.Format(_T("%s\\%s"), srcFolder, archiveChange->filename.c_str());
		else
			fileFullPath = archiveChange->filename;
		fileRelativePath = archiveChange->filename;
		...
		if( archiveChange )
		{
			...
			case MPQ_ARCHIVE_CHANGE_UPDATE:
				{
					File f0;
					f0.Open(fileFullPath.c_str(), Reuben::Platform::File::MODE_READ_EXISTING);
					if( f0.IsOpened() )
					{
						f0.Close();
						if( !InsertFileFromFile(m_file, fileRelativePath.c_str(), fileFullPath.c_str()) )
						{
							TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFFile::SavePatchData Error (2B) [Insert File error] filename=%s src=%s"), fileRelativePath.c_str(), fileFullPath.c_str());
							return FALSE;
						}
						PatchStepAdd(writeLogVersion, fileRelativePath.c_str());
					}else
					{
						TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::SavePatchData Error (3B) [source file not opened] filename=%s"), fileFullPath.c_str());
						return FALSE;
					}
				}
				break;
	...
	if(!InsertFileFromMemory(m_file, listfile_filename.c_str(), (BYTE*)tempbuf, listfile_tLength * sizeof(TCHAR), FALSE))
	{
		TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::SavePatchData Error (6B) [(listfile) save error]"));
		return FALSE;
	}
	SafeDeallocate(tempbuf);
...
BOOL GDFFile::PeekPatchInfo(PCSTRING PatchFilename)
{
	BOOL succeed = FALSE;
	FreePatch();
	File pFile;
	pFile.Open(PatchFilename, File::MODE_READ_EXISTING);
	if (pFile.IsOpened())
	{
		try
		{
		...
		{
			FreePatch();
			TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::PeekPatchInfo Error (1) [general error] PatchFilename=%s"), PatchFilename);
		}
		pFile.Close();
	}
	return succeed;
...
BOOL GDFFile::PeekPatchInfoWithList(PCSTRING PatchFilename)
{
	BOOL succeed = FALSE;
	FreePatch();
	m_file.Open(PatchFilename, File::MODE_READ_EXISTING);
	if (m_file.IsOpened())
	{
		try
		{
			...
			{
				FreePatch();
				TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::PeekPatchInfoWithList Error (1) [error reading necessary info] PatchFilename=%s"), PatchFilename);
			}
		}catch(FileException Er)
		{
			FreePatch();
			TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::PeekPatchInfo Error (2) [general error] PatchFilename=%s"), PatchFilename);
		}
		m_file.Close();
	}
	return succeed;
...
BOOL GDFFile::LoadHeader(File& pFile)
{
	...
	if( pFile.IsOpened() )
	{
		...
		{
			FreeHeader();
			TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::LoadHeader Error (1) [read error]"));
			return FALSE;
		}
	}else
	{
		TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::LoadHeader Error (2) [file not opened]"));
		return FALSE;
	}
...
BOOL GDFFile::LoadHashTable(File& pFile, UInt32 hashOffset, UInt32 hashEntries)
{
	...
	if( pFile.IsOpened() )
	{
		...
		try
		{
			pFile.SetPosition(hashOffset, Reuben::Platform::File::MoveEnum::MOVE_BEGIN);
		...
		{
			FreeHashTable();
			TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFFile::LoadHashTable Error (1) [hash table read error] hashOffset=%i, hashEntries=%i"), hashOffset, hashEntries);
		}
		SafeDeallocate(hashTableArray);
	}else
	{
		TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFFile::LoadHashTable Error (2) [file not opened] hashOffset=%i, hashEntries=%i"), hashOffset, hashEntries);
	}
	return succeed;
}
...
BOOL GDFFile::LoadBlockTable(File& pFile, UInt32 blockOffset, UInt32 blockEntries)
{
	...
	if( pFile.IsOpened() )
	{
		...
		try
		{
			pFile.SetPosition(blockOffset, Reuben::Platform::File::MoveEnum::MOVE_BEGIN);
		...
		{
			FreeBlockTable();
			TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFFile::LoadBlockTable Error (1) [block table read error] hashOffset=%i, hashEntries=%i"), blockOffset, blockEntries);
		}
		SafeDeallocate(blockTableArray);
	}else
	{
		TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFFile::LoadBlockTable Error (2) [file open error] hashOffset=%i, hashEntries=%i"), blockOffset, blockEntries);
	}
	return succeed;
}
...
BOOL GDFFile::LoadListFile()
{
	...
	if( m_file.IsOpened() )
	{
		...
		GDFStream* fstream = GetStream(listfile.c_str());
		if( fstream )
		{
			UInt32 outlen = 0;
...
//				ZeroMemory(buf, fileSize);
				...
				{
					TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::LoadListFile Error (1) [file stream read error for list file.]"));
				}
				SafeDeallocate(buf);
			}else
			{
				succeed = TRUE;
				DEBUG_INFODTL(GLOBAL_LOGGER, _T("GDFFile::LoadListFile Warning (2) [file list size=0.]"));
			}
		}else
		{
			TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::LoadListFile Error (3) [file stream not found for list file.]"));
		}
	}else
	{
		TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::LoadListFile Error (4) [file open error]"));
	}
	return succeed;
}
...
GDFStream* GDFFile::GetStream(PCSTRING TargetFilename)
{
	if( m_mode==GDFFILE_STATE_GAME )	// FILE stream mode
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::GetStream Error (1) [get stream, in game mode], TargetFilename=%s"), TargetFilename);
		return NULL;
	}

	unsigned long hashPos;
	if( FindFileInHashTable(m_hashTable, TargetFilename, 0, 0, hashPos) &&
		m_hashTable[hashPos] &&
		m_blockTable[m_hashTable[hashPos]->fileBlockIndex])
	{
		return SafeCreateObject3(GDFStream, 
			this,
...
BYTE* GDFFile::GetMapViewOfFile(PCSTRING TargetFilename, UInt32& beginOffset, UInt32& fileSize)
{
	if( m_mode!=GDFFILE_STATE_GAME )	// FILE stream mode
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::GetMapViewOfFile Error (1) [get mapview error, not in game mode], TargetFilename=%s"), TargetFilename);
		return NULL;
	}
...
BOOL GDFFile::SavePatchHashTable(File& f0, UInt32 fileOffset)
{
	...
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::SavePatchHashTable Error (1) [file write error], fileOffset=%i"), fileOffset);
	}
	SafeDeallocate(hashTableArray);
...
BOOL GDFFile::SavePatchBlockTable(File& f0, UInt32 fileOffset)
{
	...
	try
	{
		...
		f0.SetLength(f0.GetPosition());
		succeed = TRUE;
	}catch(...)
	{
		succeed = FALSE;
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::SavePatchBlockTable Error (2) [file write error], fileOffset=%i"), fileOffset);
	}
	SafeDeallocate(blockTableArray);
...
BOOL GDFFile::SavePatchHeader(File& f0)
{
	if( !f0.IsOpened() )
	{
		TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::SavePatchHeader Error (1) [file not opened]"));
		return FALSE;
	}
	try
	{
	...
	{
		TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::SavePatchHeader Error (2) [file write error]"));
		return FALSE;
	}
	return TRUE;
}
...
BOOL GDFFile::BackupHashBlock(PCSTRING bkFilename)
{
	...
	File bakfile;
	bakfile.Open(bkFilename, File::MODE_WRITE);
	if (bakfile.IsOpened())
	{
		...
		{
			bakfile.Close();
			TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::BackupHashBlock Error (1) [header write error]"));
			return FALSE;
		}
		...
		{
			succeed = FALSE;
			TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::BackupHashBlock Error (1) [file write hash error] fileOffset=%i"), fileOffset);
		}
		SafeDeallocate(hashTableArray);
		...
		try
		{
			...
			bakfile.SetLength(bakfile.GetPosition());
			succeed = TRUE;
		}catch(...)
		{
			succeed = FALSE;
			TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::BackupHashBlock Error (2) [file write block error] fileOffset=%i"), fileOffset);
		}
		SafeDeallocate(blockTableArray);
		...
		bakfile.SetPosition(0, Reuben::Platform::File::MoveEnum::MOVE_BEGIN);
		try
		{
			bakfile.Write((BYTE*)&header, sizeof(GdfBackupHeader));
		}catch(...)
		{
			bakfile.Close();
			TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::BackupHashBlock Error (2) [header write error]"));
			return FALSE;
		}
		bakfile.Close();
		return TRUE;
...
BOOL GDFFile::InsertFileFromMemory(File& f0, PCSTRING TargetFilename, BYTE* pData, UInt32 bufSize, BOOL addToListfile)	// insert or update
{
	//!!! sector ignored?
	BOOL succeed = FALSE;
	UInt32 currentBlockOffset = (UInt32)f0.GetPosition();
	UInt32 hashA = HashString(TargetFilename, MPQ_HASH_NAME_A);
	UInt32 hashB = HashString(TargetFilename, MPQ_HASH_NAME_B);
		...
		m_blockTable[m_hashTable[hashPos]->fileBlockIndex])
	{
		...
		if( !IsGdfStreamSame(TargetFilename, pData, bufSize) )
		{
			...
			}catch(...)
			{
				...
				TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromMemory Error (1) [write error] TargetFilename=%s"), TargetFilename);
			}
	...
	}else
	{
		// ADD case
		UInt tryCount = 0;
		hashPos = HashString(TargetFilename, MPQ_HASH_TABLE_OFFSET) & (m_hashTable.size() - 1);
		...
		{
			TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromMemory Error (2) [hash overloaded] TargetFilename=%s"), TargetFilename);
		}
		else
		{
			// write at the end
			...
			}catch(...)
			{
				...
				f0.SetPosition(currentBlockOffset, Reuben::Platform::File::MOVE_BEGIN);
				f0.SetLength(currentBlockOffset);
				TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromMemory Error (3) [file write error] TargetFilename=%s"), TargetFilename);
			}
...
BOOL GDFFile::InsertFileFromStream(File& f0, PCSTRING TargetFilename, GDFStream* istream)	// insert or update
{
	//!!! sector ignored?
	BOOL succeed = FALSE;
	UInt32 currentBlockOffset = (UInt32)f0.GetPosition();
		...
		m_blockTable[m_hashTable[hashPos]->fileBlockIndex])
	{
		...
		if( !IsGdfStreamSame(TargetFilename, istream) )
		{
			...
			try
			{
				...
				while( bufOffset < istream->GetFileSize() )
				{
					...
					if( bufLen > 0 )
					{
						...
						{
							TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromStream Error (1) [file read error] TargetFilename=%s"), TargetFilename);
							f0.SetPosition(currentBlockOffset, Reuben::Platform::File::MOVE_BEGIN);
							f0.SetLength(currentBlockOffset);
							succeed = FALSE;
							break;
						...
						if( testValue!=bufLen )
						{
							TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromStream Error (2) [file write error] TargetFilenaem=%s"), TargetFilename);
							f0.SetPosition(currentBlockOffset, Reuben::Platform::File::MOVE_BEGIN);
							f0.SetLength(currentBlockOffset);
							succeed = FALSE;
							break;
			...
			}catch(...)
			{
				...
				f0.SetLength(currentBlockOffset);
				succeed = FALSE;
				TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromMemory Error (3) [Write error] TargetFilename=%s"), TargetFilename);
			}
	...
	}else
	{
		...
		if( tryCount >= m_hashTable.size() )
		{
			TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromStream Error (4) [hash overloaded] TargetFilename=%s"), TargetFilename);
		}
		else
		{
			// write at the end
			...
			try
			{
				...
				while( bufOffset < istream->GetFileSize() )
				{
					...
					if( bufLen > 0 )
					{
						if( !istream->Read(buf, bufLen, outlen, STREAM_BUF_MAX) )
						{
							TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromStream Error (5) [file read error] TargetFilename=%s"), TargetFilename);
							f0.SetPosition(currentBlockOffset, Reuben::Platform::File::MOVE_BEGIN);
							f0.SetLength(currentBlockOffset);
							succeed = FALSE;
							break;
						...
						if( testValue!=bufLen )
						{
							TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromStream Error (6) [file write error] TargetFilename=%s"), TargetFilename);
							f0.SetPosition(currentBlockOffset, Reuben::Platform::File::MOVE_BEGIN);
							f0.SetLength(currentBlockOffset);
							succeed = FALSE;
							break;
			...
			{
				TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromStream Error (7) [write error] TargetFilename=%s"), TargetFilename);
				f0.SetPosition(currentBlockOffset, Reuben::Platform::File::MOVE_BEGIN);
				f0.SetLength(currentBlockOffset);
				succeed = FALSE;
			}
...
BOOL GDFFile::InsertFileFromFile(File& f0, PCSTRING TargetFilename, PCSTRING SourcePath)	// insert or update
{
	//!!! sector ignored?
	BOOL succeed = FALSE;
	UInt32 currentBlockOffset = (UInt32)f0.GetPosition();
		...
		m_blockTable[m_hashTable[hashPos]->fileBlockIndex])
	{
		...
		{
			File fSrc;
			fSrc.Open(SourcePath, Reuben::Platform::File::MODE_READ_EXISTING);
			if( fSrc.IsOpened() )
			{
				...
				try
				{
					...
					while( bufOffset < filesize )
					{
						...
						if( bufLen > 0 )
						{
							...
							{
								TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromFile Error (1) [file read error] TargetFilename=%s"), TargetFilename);
								f0.SetPosition(currentBlockOffset, Reuben::Platform::File::MOVE_BEGIN);
								f0.SetLength(currentBlockOffset);
								succeed = FALSE;
								break;
							...
							if( testValue!=bufLen )
							{
								TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromFile Error (2) [file write error] TargetFilename=%s"), TargetFilename);
								f0.SetPosition(currentBlockOffset, Reuben::Platform::File::MOVE_BEGIN);
								f0.SetLength(currentBlockOffset);
								succeed = FALSE;
								break;
				...
				{
					TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromFile Error (3) [write error] TargetFilename=%s"), TargetFilename);
					f0.SetPosition(currentBlockOffset, Reuben::Platform::File::MOVE_BEGIN);
					f0.SetLength(currentBlockOffset);
					fSrc.Close();
					succeed = FALSE;
			...
			{
				TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromFile Error (4) [source file open error] TargetFilename=%s"), TargetFilename);
				f0.SetPosition(currentBlockOffset, Reuben::Platform::File::MOVE_BEGIN);
				f0.SetLength(currentBlockOffset);
				succeed = FALSE;
			}
	...
	}else
	{
		...
		if( tryCount >= m_hashTable.size() )
		{
			TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromFile Error (5) [hash overloaded] TargetFilename=%s"), TargetFilename);
		}
		else
		{
			// write at the end
			...
			if( fSrc.IsOpened() )
			{
				...
				try
				{
					...
					while( bufOffset < filesize )
					{
						...
						if( bufLen > 0 )
						{
							if( !fSrc.Read(buf, bufLen) )
							{
								TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromFile Error (6) [file read error] TargetFilename=%s SourcePath=%s"), TargetFilename, SourcePath);
								f0.SetPosition(currentBlockOffset, Reuben::Platform::File::MOVE_BEGIN);
								f0.SetLength(currentBlockOffset);
								succeed = FALSE;
								break;
							...
							if( testValue!=bufLen )
							{
								TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromFile Error (7) [file write error] TargetFilename=%s SourcePath=%s"), TargetFilename, SourcePath);
								f0.SetPosition(currentBlockOffset, Reuben::Platform::File::MOVE_BEGIN);
								f0.SetLength(currentBlockOffset);
								succeed = FALSE;
								break;
				...
				{
					TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromFile Error (8) [write error] TargetFilename=%s SourcePath=%s"), TargetFilename, SourcePath);
					f0.SetPosition(currentBlockOffset, Reuben::Platform::File::MOVE_BEGIN);
					f0.SetLength(currentBlockOffset);
					fSrc.Close();
					succeed = FALSE;
			...
			{
				TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromFile Error (9) [source file open error] TargetFilename=%s SourcePath=%s"), TargetFilename, SourcePath);
				succeed = FALSE;
			}
		}
	}
...
BOOL GDFFile::DeleteFile(File& f0, PCSTRING TargetFilename)
{
	BOOL succeed;
	succeed = DeleteFile(TargetFilename, MPQNeutral, 0);
	if( succeed )
	{
		// (listfile) manipulation
		for( ListFileVector::iterator itr = m_listFile.begin(); itr!=m_listFile.end(); ++itr )
		{
			if( (*itr)->filename.CompareNoCase(TargetFilename)==0 )
			{
...
BOOL GDFFile::IsGdfFileExist(PCSTRING TargetFilename)
{
...
BOOL GDFFile::IsGdfStreamSame(PCSTRING TargetFilename, PCSTRING FilePath)
{
	...
	File f0;
	BOOL same = TRUE;
	f0.Open(FilePath, Reuben::Platform::File::MODE_READ_EXISTING);
	if( f0.IsOpened() )
	{
	...
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::IsGdfStreamSame Error (1) [error load file] FilePath=%s"), FilePath);
	}
	m_file.SetPosition(currentOffset, Reuben::Platform::File::MOVE_BEGIN);
	return FALSE;
}

BOOL GDFFile::IsGdfStreamSame(PCSTRING  TargetFilename, GDFStream* stream2)
{
...
BOOL GDFFile::IsGdfStreamSame(PCSTRING TargetFilename, BYTE* pData, UInt32 bufSize)
{
	GDFStream* stream1 = GetStream(TargetFilename);
	TRACE_ENSURE(stream1);
...
BOOL GDFFile::LoadFile(PCSTRING PatchFilename)		// continuous file stream
{
	m_mode	= GDFFILE_STATE_GAME;
	if( !File::IsExists(PatchFilename) )
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::LoadFile Error (3) [file not found] PatchFilename=%s"), PatchFilename);
		return FALSE;
	}
	...
	if( succeed )
	{
		m_hFile = CreateFile(PatchFilename, FILE_READ_DATA, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_READONLY, NULL);
		m_hMapFile = CreateFileMapping(m_hFile, NULL, PAGE_READONLY, 0, 0, PatchFilename);
		if( m_hMapFile!=NULL )
		{
//			m_pInFile = (Byte *)MapViewOfFile(m_hMapFile, FILE_MAP_READ, 0, 0, 1000000000);
		...
		{
			succeed = FALSE;
			TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::LoadFile Error (1) [Create File Mapping failed] PatchFilename=%s"), PatchFilename);
		}
		if( !succeed )
			FreePatch();
	}
	if( succeed )
	{
		...
		m_backup_filename = GetBackupFilename(PatchFilename);
	}
	return succeed;
}

BOOL GDFFile::LoadFileWithList(PCSTRING PatchFilename)
{
	m_mode = GDFFILE_STATE_PATCH_READ;
	if( !File::IsExists(PatchFilename) )
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::LoadFileWithList Error (3) [file not found] PatchFilename=%s"), PatchFilename);
		return FALSE;
	}
	...
	if( succeed )
	{
		try
		{
			m_file.Open(PatchFilename, Reuben::Platform::File::MODE_READ_EXISTING);
			if( !m_file.IsOpened() )
			{
				succeed = FALSE;
				TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::LoadFileWithList Error (1) [error on open] PatchFilename=%s"), PatchFilename);
			}
		}catch(...)
		{
			succeed = FALSE;
			TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::LoadFileWithList Error (2) [error on open] PatchFilename=%s"), PatchFilename);
		}
		if( !succeed )
			FreePatch();
	}
...
BOOL GDFFile::LoadFileWrite(PCSTRING PatchFilename, UInt32 defaultHashEntries_power2, PCSTRING revertFilename)
{
	BOOL succeed = TRUE;
	m_mode = GDFFILE_STATE_MASTER_WRITE;
	String backup_filename = GetBackupFilename(PatchFilename);
	if( Reuben::Platform::File::IsExists(backup_filename.c_str()) )
	{
		m_backup_filename = backup_filename;
		try
		{
			m_header = SafeCreateObject(GdfHeader);
			m_file.Open(PatchFilename, Reuben::Platform::File::MODE_READ_WRITE_EXISTING);
			if( !m_file.IsOpened() )
			{
				succeed = FALSE;
				TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::LoadFileWrite Error (1) [error on open] PatchFilename=%s"), PatchFilename);
			}else
			{
				if( !RevertFromBackup(m_backup_filename.c_str()) )
				{
					TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::LoadFileWrite Error (2) [revert failed] PatchFilename=%s"), PatchFilename);
					succeed = FALSE;
				}else
					DeleteBackup();
				m_file.Close();
		...
		{
			succeed = FALSE;
			TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::LoadFileWrite Error (3) [error on open] PatchFilename=%s"), PatchFilename);
		}
	}
	if( succeed && !Reuben::Platform::File::IsExists(PatchFilename) )
	{
		if( defaultHashEntries_power2 > 30 )
		{
			succeed = FALSE;
			TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFFile::LoadFileWrite Error (4) [new file without parameter] PatchFilename=%s, defaultHashEntries_power2=%i"), PatchFilename, defaultHashEntries_power2);
		}else
		{
			...
			{
				// !!
				TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFFile::LoadFileWrite Error (5) [new file failed] PatchFilename=%s, defaultHashEntries_power2=%i"), PatchFilename, defaultHashEntries_power2);
			}else
			{
			}
		}
	...
	if( succeed )
	{
		...
		if( succeed )
		{
			try
			{
				...
				{
					succeed = FALSE;
					TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::LoadFileWrite Error (6) [error on open] PatchFilename=%s"), PatchFilename);
				}
			}catch(...)
			{
				succeed = FALSE;
				TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::LoadFileWrite Error (7) [error on open] PatchFilename=%s"), PatchFilename);
			}
			if( !succeed )
				FreePatch();
		}
...
BOOL GDFFile::CloseFile()
{
	...
	{
		TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::CloseFile Error (1) [error on close or file check]"));
		return FALSE;
	}
	return TRUE;
}
...
BOOL GDFFile::UpdateFromStream(PatchStepMap& patchstep, GDFFile* SrcGdfFile, BOOL mergePatchStep)
{
	...
	if( BackupHashBlock(m_backup_filename.c_str()) &&
	...
	else
	{
		if( !RevertFromBackup(m_backup_filename.c_str()) )
		{
			TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::UpdateFromStream Error (1) [update failed AND revert failed]"));
		}
	}
	DeleteBackup();
...
BOOL GDFFile::UpdateFromFile(ArchiveChangeVector& change, PCSTRING srcFolder, UInt32 writeLogVersion)
{
	BOOL succeed = FALSE;

	if( BackupHashBlock(m_backup_filename.c_str()) &&
		SavePatchHeader(m_file) &&
		SavePatchData(change, srcFolder, writeLogVersion) &&
		SavePatchHashTable(m_file, (UInt32)m_file.GetPosition()) &&
		SavePatchBlockTable(m_file, (UInt32)m_file.GetPosition()) &&
		SavePatchHeader(m_file)
		)
...
BOOL GDFFile::RevertFromBackup(PCSTRING bkFilename)
{
	TRACE_ENSURE(m_header);
	...
	File bakfile;
	bakfile.Open(bkFilename, File::MODE_READ_EXISTING);
	if (bakfile.IsOpened())
	{
		// backup file dummy header
		GdfBackupHeader header;
		...
		if( header.bFileCompleted==1 ) // 0: incomplete backup data, 1: complete backup data
		{
				...
				LoadBlockTable(bakfile, (UInt32)bakfile.GetPosition(), header.blockSize) )
			{
				...
				{
					succeed = FALSE;
					TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::RevertFromBackup Error (4) [Backup data write error]"));
				}
			}else
			{
				succeed = FALSE;
				TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::RevertFromBackup Error (3) [Backup data read error]"));
			}
		}else
		{
			succeed = FALSE;
			TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::RevertFromBackup Error (1) [Revert failed due to corruption of backup data]"));
		}
		bakfile.Close();
	}else
	{
		succeed = FALSE;
		TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::RevertFromBackup Error (2) [Revert failed due to missing backup data]"));
	}
	return succeed;
}
...
BOOL GDFFile::ReadStream(GDFStreamData* streamdata, BYTE* OutBuffer, UInt32 readLength, UInt32& OutLength, UInt32 MaxLength)
{
	if( m_mode!=GDFFILE_STATE_GAME )	// FILE stream mode
	{
		try
		{
			...
			if( reallen > 0 )
			{
				...
				m_file.SetPosition(m_blockTable[streamdata->m_blockIndex]->blockOffset + streamdata->m_managerBlockOffset, Reuben::Platform::File::MoveEnum::MOVE_BEGIN);
	...
	{
		TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::ReadStream (1) [View mode cannot use this function.]"));
		return FALSE;
	}
}

BOOL GDFFile::NewFile(PCSTRING PatchFilename, UInt32 defaultHashEntries_power2)
{
	FreePatch();

	if( Reuben::Platform::File::IsExists(PatchFilename) )
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::NewFile Error (1) [Filename exists, file create skipped] PatchFilename=%s"), PatchFilename);
		return FALSE;
	}
	...
	BOOL succeed = TRUE;
	File f0;
	f0.Open(PatchFilename, Reuben::Platform::File::MODE_WRITE);
	if( f0.IsOpened() )
	{
		...
		if( succeed )
		{
			UInt32 numHash = (UInt32)pow(2, defaultHashEntries_power2);
			...
			succeed = InsertFileFromMemory(f0, listfilename.c_str(), (BYTE*)out, listfilelen * sizeof(TCHAR), FALSE);
			if( succeed )
			{
				...
				if( succeed )
				{
					...
					if( succeed )
					{
						...
						succeed = SavePatchHeader(f0);
						if( !succeed )
							TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::NewFile Error (2) [rewrite header error]"));
					}else
						TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::NewFile Error (3) [hash write error]"));
				}else
					TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::NewFile Error (4) [block write error]"));
			}else
				TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::NewFile Error (5) [list of file write error]"));
		}else
			TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::NewFile Error (6) [header write error]"));
	...
	if( !succeed )	
	{
		Reuben::Platform::File::Delete(PatchFilename);
	}
...
BOOL GDFFile::IncreaseHash(UInt32 newhashsize)
{
	...
	UInt32 newrealsize = (UInt32)pow(2, newhashsize);
	...
	for( Index i = C_INDEX(0); i < m_listFile.size(); ++i )
	{
		...
		if( listfile )
		{
			if( FindFileInHashTable(m_hashTable, listfile->filename.c_str(), 0, 0, hashPos) && m_hashTable[hashPos])
			{
				...
				{
					TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFFile::InsertFileFromMemory Error (2) [hash overloaded] TargetFilename=%s, hashSize=%i"), listfile->filename.c_str(), (UInt)newhashtable.size());
					succeed = FALSE;
				}
				else
				{
			...
			{
				TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::IncreaseHash Error (2) [memory not found in original hash table, increase failed] filename=%s"), listfile->filename.c_str());
				succeed = FALSE;
				break;
			}
		}else
		{
			TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::IncreaseHash Error (3) [empty listfile found, increase failed] filename=%s"));
			succeed = FALSE;
			break;
		}
	}
...
BOOL GDFFile::TestFile(PCSTRING patchFilename)
{
	if( !LoadFileWithList(patchFilename) )
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::TestFile Error (1) [load file with list error] patchFilename=%s"), patchFilename);
		return FALSE;
	}
	...
	if( m_file.GetLength()!=m_header->archiveSize )
	{
		...
		TRACE_ERRORDTL_3(GLOBAL_LOGGER, _T("GDFFile::TestFile Error (2) [file size does not match] patchFilename=%s, filesize=%i, archiveSize=%i"), patchFilename, (UInt32)m_file.GetLength(), archivesize);
		CloseFile();
		return FALSE;
	}
	...
	for( Index i = C_INDEX(0); i < m_listFile.size(); ++i )
	{
		...
		if( list )
		{
			if( IsGdfFileExist(list->filename.c_str()) )
			{
				fstream = GetStream(list->filename.c_str());
				if( fstream )
				{
					if( fstream->GetFileSize() > m_header->archiveSize )
					{
						TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFFile::TestFile Error (1) [file size is not reasonable] patchFilename=%s, size=%i"), patchFilename, fstream->GetFileSize());
						CloseFile();
						SafeDeleteObject(fstream);
						return FALSE;
					}
				...
				{
					TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::TestFile Error (1) [file in list does not exist] patchFilename=%s"), patchFilename);
					CloseFile();
					return FALSE;
				}
			}else
		...
		{
			TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFFile::TestFile Error (2) [list file content error] patchFilename=%s"), patchFilename);
			CloseFile();
			return FALSE;
		}
	}
...
BOOL GDFFile::PatchStepAdd(UInt32 writeLogVersion, PCSTRING filename)
{
...
BOOL GDFFile::PatchStepDelete(UInt32 writeLogVersion, PCSTRING filename)
{
	if( writeLogVersion==0 )	return FALSE;	// version 0 is an exception case, for not versioning anything here
...
BOOL GDFFile::LoadPatchStep()
{
	...
	if( m_file.IsOpened() )
	{
		...
		GDFStream* fstream = GetStream(patchstep.c_str());
		if( fstream )
		{
			UInt32 outlen = 0;
			UInt32 fileSize = fstream->GetFileSize();
...
//				ZeroMemory(buf, fileSize);
				if( fstream->Read((BYTE*)buf, fileSize, outlen, fileSize) )
				{
					...
					while (fullFile.npos != pos && fullFile.npos != lastPos)
					{
						...
						case ':':			// e.g. ":340" indicates this is version 340
							...
							break;
						case 'A':
							PatchStepAdd(readVer, wholeline.Right(wholeline.GetLength()-2).c_str());
							break;
						case 'D':			// e.g. "A Data/addfilename.dat" or "D Data/deletefilename.dat"
							PatchStepDelete(readVer, wholeline.Right(wholeline.GetLength()-2).c_str());
							break;
						}
						lastPos = fullFile.find_first_not_of(delim.c_str(), pos);
						pos = fullFile.find_first_of(delim.c_str(), lastPos);
				...
				{
					TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::LoadPatchStep Error (1) [file stream read error for patch step file.]"));
				}
				SafeDeallocate(buf);
			}else
			{
				succeed = TRUE;
				DEBUG_INFODTL(GLOBAL_LOGGER, _T("GDFFile::LoadPatchStep Warning (2) [patch step list size=0.]"));
			}
		}else
		{
			succeed = TRUE;
			TRACE_INFODTL(GLOBAL_LOGGER, _T("GDFFile::LoadPatchStep Warning (3) [file stream not found for patch step list.]"));
		}
	}else
	{
		TRACE_ERRORDTL(GLOBAL_LOGGER, _T("GDFFile::LoadPatchStep Error (4) [file open error]"));
	}
	return succeed;
}

CONST String& GDFFile::GetBackupFilename(PCSTRING PatchFilename)
{
	static String dirfile;
	String temp(PatchFilename);
	TCHAR delim[2] = _T("\\");
	temp.Replace(_T("/"), _T("\\"));
	String::size_type lastPos = temp.find_last_of(delim);
	if( lastPos==String::npos )
	{
		dirfile.Format(_T("_%s.tmp"), PatchFilename);
		return dirfile;
	}else
	{
		String dir(temp.substr(0, lastPos));
		String filename(temp.substr(lastPos+1, temp.GetLength()-(lastPos+1)));
		dirfile.Format(_T("%s\\_%s.tmp"), dir.c_str(), filename.c_str() );
		return dirfile;
	}
}

BOOL GDFFile::GetBlockStat(UInt32& numAllBlock, UInt32& numUsedBlock, UInt32& sizeAllBlock, UInt32& sizeUsedBlock)
{
	UInt32 numA = 0, numB = 0, numC = 0, numD = 0;
	...
	for( Index i = C_INDEX(0); i < m_hashTable.size(); ++i )
	{
		...
		if( hash )
		{
			if( hash->fileBlockIndex!=MPQ_HASH_ENTRY_EMPTY && hash->fileBlockIndex!=MPQ_HASH_ENTRY_DELETED 
				&& hash->fileBlockIndex < m_hashTable.size()
				&& m_blockTable[hash->fileBlockIndex] )
			{
				block = m_blockTable[hash->fileBlockIndex];
				if( block )
				{
					++numB;
					numD += block->blockSize;
				}
			}
		}
	}
