...
BOOL GDFManager::LoadPatchFile(PCSTRING PatchFilename, GDFFILE_STATE fileState)
{
	BOOL succeed;
	switch(fileState)
	{
...
BOOL GDFManager::Merge(PCSTRING ToFilename, PCSTRING FromFilename, BOOL inheritPatchStep, PCSTRING revertFilename)
{
	BOOL succeed = TRUE;
	GDFFile gdfMaster;
	GDFFile gdfPatch;
	...
	{
		UInt32 predictedPower2 = 0;
		if( !File::IsExists(ToFilename) )
		{
			UInt32 hashSize = gdfPatch.GetHashSize();
		...
		if( gdfMaster.LoadFileWrite(ToFilename, predictedPower2) )
		{
			// backup a revert file if NOT NULL and NOT EXIST
			if( revertFilename && !Reuben::Platform::File::IsExists(revertFilename) )
				gdfMaster.BackupHashBlock(revertFilename);
			...
			{
				TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFManager::Merge Error (1) [UpdateFromStream failed] masterFile=%s, patchFile=%s"), ToFilename, FromFilename);
				succeed = FALSE;
			}
			// destroy rubbish
			gdfMaster.CloseFile();
		...
		{
			TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFManager::Merge Error (3) [master file read error] masterFile=%s"), ToFilename);
			succeed = FALSE;
		}
		gdfPatch.CloseFile();
	}else
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFManager::Merge Error (2) [patch file read error] patchFile=%s"), FromFilename);
		succeed = FALSE;
	}
	return succeed;
}

BOOL GDFManager::UpdatePatch(PCSTRING patchFilename, ArchiveChangeVector& change, PCSTRING srcFolder, UInt32 writeLogVersion, BOOL ForceMaster)
{
	BOOL succeed;
	...
	if( succeed )
	{
		succeed = masterFile.UpdateFromFile(change, srcFolder, writeLogVersion);
	}
...
GDFStream* GDFManager::GetStream(PCSTRING TargetFilename)
{
	return m_pMainFile.GetStream(TargetFilename);
}

BOOL GDFManager::TestPatchFile(PCSTRING patchFilename)
{
	return m_pMainFile.TestFile(patchFilename);
}

BOOL GDFManager::Repatch(PCSTRING patchFilename)
{
	String ToFilename;
	ToFilename.Format(_T("%s.trn.tmp"), patchFilename);
	if( File::IsExists(ToFilename.c_str()) )
	{
		BOOL found = FALSE;
		for( Index i = C_INDEX(0); i < C_INDEX(10000); ++i )
		{
			ToFilename.Format(_T("%s%03i.trn.tmp"), patchFilename, i);
			if( !File::IsExists(ToFilename.c_str()) )
			{
				found = TRUE;
				break;
		...
		if( !found )
		{
			TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFManager::Merge Repatch (1) [Suitable temp filename not found] SrcFile=%s"), patchFilename);
			return FALSE;
		}
	}

	if( !Merge(ToFilename.c_str(), patchFilename, TRUE, NULL) )
	{
		TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFManager::Merge Repatch (2) [Repatch failed] SrcFile=%s, TargetFile=%s"), patchFilename, ToFilename.c_str());
		return FALSE;
	}else
	{
		if( MoveFileEx(ToFilename.c_str(), patchFilename, MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH) )
		{
			return TRUE;
		}else
		{
			TRACE_ERRORDTL_2(GLOBAL_LOGGER, _T("GDFManager::Merge Repatch (3) [Move failed] SrcFile=%s, TargetFile=%s"), patchFilename, ToFilename.c_str());
			return FALSE;
		}
	}
}

BYTE* GDFManager::GetMapViewOfFile(PCSTRING TargetFilename, UInt32& beginOffset, UInt32& fileSize)
{
	return m_pMainFile.GetMapViewOfFile(TargetFilename, beginOffset, fileSize);
}

BOOL GDFManager::GetBlockStat(UInt32& numAllBlock, UInt32& numUsedBlock, UInt32& sizeAllBlock, UInt32& sizeUsedBlock)
{
	return m_pMainFile.GetBlockStat(numAllBlock, numUsedBlock, sizeAllBlock, sizeUsedBlock);
}

BOOL GDFManager::Revert(PCSTRING ToFilename, PCSTRING revertFilename)
{
	// Warning: Revert is actually a destruction, that re-upgrade is required.
	// No undo can be done.
	BOOL succeed = TRUE;
	GDFFile gdfMaster;
	if( gdfMaster.LoadFileWrite(ToFilename, 0) )
	{
		if( revertFilename && Reuben::Platform::File::IsExists(revertFilename) )
		{
			gdfMaster.RevertFromBackup(revertFilename);
		}else
		{
			TRACE_WARNDTL_1(GLOBAL_LOGGER, _T("GDFManager::Revert Warning (1) [revert read error] revertFilename=%s"), revertFilename);
			succeed = FALSE;
		}
		gdfMaster.CloseFile();
	}else
	{
		TRACE_ERRORDTL_1(GLOBAL_LOGGER, _T("GDFManager::Revert Error (2) [master file read error] masterFile=%s"), ToFilename);
		succeed = FALSE;
	}
	return TRUE;
}
