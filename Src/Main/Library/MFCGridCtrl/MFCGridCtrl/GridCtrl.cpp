...
//}}AFX_MSG_MAP
	...
	// Add by Wong Cheung
	ON_NOTIFY(GVN_ONCOMBOCHANGE, IDC_INPLACE_CONTROL, OnComboChange)
	//ON_NOTIFY(GVN_ONCHECKCLICK, IDC_INPLACE_CONTROL, OnCheckClick)
	ON_NOTIFY(GVN_ONEDITEND, IDC_INPLACE_CONTROL, OnEditEnd)
END_MESSAGE_MAP()
...
// Add by Wong Cheung to handle the combo change message
void CGridCtrl::OnComboChange(NMHDR *pNMHDR, LRESULT *pResult)
{
	GV_DISPINFO *pgvDispInfo = (GV_DISPINFO *)pNMHDR;
    GV_ITEM     *pgvItem = &pgvDispInfo->item;
	...
	pOwner->SendMessage(WM_GRIDCOMBOCHANGE, GetDlgCtrlID(), (LPARAM) pgvItem);

	*pResult = 0;
}

// Add by Wong Cheung to handle the check box click event
void CGridCtrl::OnCheckClick(int row,int col,BOOL bCheck)
{
	CWnd * pOwner = GetOwner();
	GV_CHECKBOXINFO info;
	info.row = row;
	info.col = col;
	info.check = bCheck;
	if (IsWindow(pOwner->GetSafeHwnd()))
	pOwner->SendMessage(WM_GRIDCHECKCLICK, GetDlgCtrlID(), (LPARAM)&info);
}

// Add by Wong Cheung to handle the edit end of a text box
void CGridCtrl::OnEditEnd(NMHDR *pNMHDR, LRESULT *pResult)
{
	GV_DISPINFO *pgvDispInfo = (GV_DISPINFO *)pNMHDR;
    GV_ITEM     *pgvItem = &pgvDispInfo->item;
	...
	CWnd* pOwner = GetOwner();
	if (IsWindow(pOwner->GetSafeHwnd()))
	pOwner->SendMessage(WM_GRIDEDITEND, GetDlgCtrlID(), (LPARAM) pgvItem);
                                                  ...
                                                  BOOL bForceRecalculation /*=FALSE*/)
{
    ...
    int maxVisibleCol = min(col, GetColumnCount() - 1);
    if (pRect)
    {
...
CCellRange CGridCtrl::GetUnobstructedNonFixedCellRange(BOOL bForceRecalculation /*=FALSE*/)
{
    ...
    {
        bottom += GetRowHeight(row);
        if (bottom >= rect.bottom)
            break;
    }
    int maxVisibleRow = min(row, GetRowCount() - 1);
