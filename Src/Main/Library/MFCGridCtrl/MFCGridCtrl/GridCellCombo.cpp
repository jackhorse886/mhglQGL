						   ...
						   UINT nFirstChar)
{
	...
	// Store the cell : Add by Wong Cheung
	m_pCell = (CGridCellCombo*) (((CGridCtrl*)pParent)->GetCell(nRow,nColumn));
...
void CInPlaceList::EndEdit()
{
	...
	// Store the changed index : Add by Wong Cheung
	if (m_pCell)
		m_pCell->GetIndex();
...
void CInPlaceList::OnSelchange() 
{
	...
	dispinfo.hdr.hwndFrom = GetSafeHwnd();
	dispinfo.hdr.idFrom = GetDlgCtrlID();
	dispinfo.hdr.code = GVN_ONCOMBOCHANGE; // you can define a new message in GridCtrl and 
...
BOOL CGridCellCombo::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
	...
	// Set the preset Index to combo : Add by Wong Cheung
	if (m_nStringIndex != -1)
		((CInPlaceList*)m_pEditWnd)->SetCurSel(m_nStringIndex);
...
void CGridCellCombo::EndEdit()
{
	...
	{
		// Store the changed index : Add by Wong Cheung
		m_nStringIndex = ((CInPlaceList*)m_pEditWnd)->GetCurSel();
...
// Retrieve and Set the combo index : Add by Wong Cheung
void CGridCellCombo::SetIndex(int nIndex)
{
	if (nIndex < 0) 
		nIndex = 0;
	if (nIndex >= m_Strings.GetSize())
		nIndex = m_Strings.GetSize() - 1;

	m_nStringIndex = nIndex;
	if (m_pEditWnd)
		((CInPlaceList*)m_pEditWnd)->SetCurSel(m_nStringIndex);
	else
		SetText(m_Strings.GetAt(m_nStringIndex));
}

int CGridCellCombo::GetIndex()
{
	if (m_pEditWnd)
		 m_nStringIndex = ((CInPlaceList*)m_pEditWnd)->GetCurSel();
	return m_nStringIndex;
}

int	CGridCellCombo::GetTotalCount()
{
