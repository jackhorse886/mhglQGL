...
// Add by Wong Cheung, check box info structure
typedef struct tagGV_CHECKBOXINFO {
	int row;
	int col;
	int check;
} GV_CHECKBOXINFO;
...
#define GVN_ONCOMBOCHANGE		LVN_LAST + 1
#define GVN_ONCHECKCLICK		GVN_ONCOMBOCHANGE + 1
#define GVN_ONEDITEND			GVN_ONCHECKCLICK + 1

// Add by Wong Cheung User message for combo change
#define WM_GRIDCOMBOCHANGE		WM_USER + 2000
#define WM_GRIDCHECKCLICK		WM_GRIDCOMBOCHANGE + 1
#define WM_GRIDEDITEND			WM_GRIDCHECKCLICK + 1
...
public:
	...
	// Add by Wong Cheung to handle check box click message
	void OnCheckClick(int row,int col,BOOL bCheck);
...
protected:
	...
	// Add by Wong Cheung
	afx_msg void OnComboChange(NMHDR* pNMHDR, LRESULT* pResult);
	// Add by Wong Cheung
